...
void Maze::createEmptyMazeAndSetInitialPoint()
{
  maze.clear();
...
bool Maze::stuck()
{
  if((myx-2 <= 0  || maze[myy][myx-2].visited) && (myx+2 >= width  || maze[myy][myx+2].visited))
  {
    if((myy-2 <= 0  || maze[myy-2][myx].visited) && (myy+2 >= height || maze[myy+2][myx].visited ))
    {
      return 1;
    }
  }
...
void Maze::createMaze()
{
  nrOfMazes ++;
  clearStack(stck);
  createEmptyMazeAndSetInitialPoint();
  bool endGame = false;
  while(!endGame)
  {
    if(stuck())
    {
      if(availableFields())//(stck.size()>=2 it is more "random"
      {
        goBack();
        continue;
      }
